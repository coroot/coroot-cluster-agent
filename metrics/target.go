...
func (t *Target) StartExporter(reg *prometheus.Registry, credentials Credentials, scrapeInterval, scrapeTimeout time.Duration, changeEmitter *emitter.ChangeEmitter) error {
	...
	case TargetTypePostgres:
		...
		collector, err := postgres.New(dsn, scrapeInterval, collectTimeout, t.logger, changeEmitter, t.Addr)
		if err != nil {
			return err
		}
		t.coll = collector
